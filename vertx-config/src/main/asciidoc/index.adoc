...
* a **Configuration store** defines a location from where the configuration data is read and also a format (JSON by default)
...
If ssl/tls connection is used, trust-relations must be specified:

==== Trust all

This configuration is not recommended and only mentioned for completeness.
...
==== Usage of JKS trust store

[source, $lang]
----
put("trustStoreOptions", new JsonObject().put("path", path).put("password", password))
----

==== Usage of Pfx trust store

[source, $lang]
----
put("pfxTrustStoreOptions", new JsonObject().put("path", path).put("password",password))
----
